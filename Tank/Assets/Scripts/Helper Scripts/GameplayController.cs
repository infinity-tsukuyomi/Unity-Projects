...
using UnityEngine.SceneManagement; // to use pause button
...
public class GameplayController : MonoBehaviour {
	...
	public float min_ObstacleDelay = 10f, max_ObstacleDelay = 40f; // how much we're gonna wait to spawn obstacle
	private float halfGroundSize; // the half of the ground size (full ground = 200, half = 100)
	...
	IEnumerator GenerateObstacles() {
		float timer = Random.Range (min_ObstacleDelay, max_ObstacleDelay) / playerController.speed.z; // based on player's speed will be generated the obstacles
		yield return new WaitForSeconds (timer);

		CreateObstacles (playerController.gameObject.transform.position.z + halfGroundSize); // The obstacles will be generated halfway down the road from the player

		StartCoroutine ("GenerateObstacles"); // calling the coroutine inside of the function will create an infinite coroutine 
	}
	...
	void CreateObstacles(float zPos) {
		...
		if (0 <= r && r < 7) {
			int obstacleLane = Random.Range (0, lanes.Length); // if spawn the obstacle on the left - zombie'll be spawned on the right
	...
	void AddZombies(Vector3 pos) {
		...
		for (int i = 0; i < count; i++) {
			Vector3 shift = new Vector3 (Random.Range(-0.5f, 0.5f), 0f, Random.Range(1f, 10f) * i); // shifting the position

			Instantiate (zombiePrefabs[Random.Range(0, zombiePrefabs.Length)], // spawn random zombie from the array
	...
	public void IncreaseScore() {
		zombie_Kill_Count++;
		score_Text.text = zombie_Kill_Count.ToString (); // converting int to string
	}
